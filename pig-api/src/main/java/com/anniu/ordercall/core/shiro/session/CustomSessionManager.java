package com.anniu.ordercall.core.shiro.session;


import com.anniu.ordercall.bean.bo.user.UserOnlineBo;
import com.anniu.ordercall.core.utils.LoggerUtils;
import com.anniu.ordercall.core.utils.StringUtils;
...
public class CustomSessionManager {
     ...
     * session status
     */
    public static final String SESSION_STATUS = "anniu——ordercall-online-status";
    ...
    private UserOnlineBo getSessionBo(Session session) {
        ...
        if (obj instanceof SimplePrincipalCollection) {
            ...
            if (null != obj && obj instanceof User) {
                ...
                //是否踢出
                com.anniu.ordercall.core.shiro.session.SessionStatus sessionStatus = (com.anniu.ordercall.core.shiro.session.SessionStatus) session.getAttribute(SESSION_STATUS);
    ...
    public Map<String, Object> changeSessionStatus(Boolean status, String sessionIds) {
        ...
        try {
            ...
            for (String id : sessionIdArray) {
                ...
                com.anniu.ordercall.core.shiro.session.SessionStatus sessionStatus = new com.anniu.ordercall.core.shiro.session.SessionStatus();
    ...
    public void forbidUserById(Long id, Long status) {
        ...
        for (UserOnlineBo bo : getAllUser()) {
            Long userId = (long) bo.getId();
            //匹配用户ID
            if (userId.equals(id)) {
                //获取用户Session
                Session session = shiroSessionRepository.getSession(bo.getSessionId());
                //标记用户Session
                com.anniu.ordercall.core.shiro.session.SessionStatus sessionStatus = (SessionStatus) session.getAttribute(SESSION_STATUS);
